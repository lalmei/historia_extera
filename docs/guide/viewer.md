...
Under the dev server the header carries a **New world** button (`#/new`): give it a seed,
a span of years and a starting civilization count, and the dev server runs the CLI and
opens the result in place — no reload, no second terminal. The same panel appears on the
"no world to show" screen, so a fresh checkout can simulate its way out of an empty
`viewer/public/worlds/`.
...
writes `worlds/world-s<seed>-y<years>-c<civs>.json` and points `?world=` at it, so
reloading — or sharing the URL — comes back to the same history. `--size` and `--raster`
are not exposed and stay at the CLI's defaults, so a world generated here is identical to
the same seed generated by `make generate`.

**Development only.** The endpoint is a Vite dev middleware
(`viewer/dev/world-generator.mjs`), and the UI is gated on `import.meta.env.DEV`, so both
are compiled out of `astro build`. A built viewer is still a static bundle with no server
behind it.

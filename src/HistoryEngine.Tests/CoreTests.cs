...
public sealed class ConfigTests
{
    ...
    public void EveryConfigFieldIsAccountedFor()
    {
        var simulationAffecting = new[]
        {
            ...
            // One field carrying two numbers, and contributing only when it is not the standard
            // year — again, see WorldConfig.ConfigHash.
            nameof(WorldConfig.Calendar),
        };
    ...
    public void HashChangesWithSimulationAffectingFields()
    {
        ...
        // Both halves of the calendar, since a world of 360 days in twelve parts and one of 360
        // days in four are different histories the moment anything is seasonal.
        Assert.NotEqual(
            baseline.ConfigHash, (baseline with { Calendar = new Calendar(240) }).ConfigHash);
        Assert.NotEqual(
            baseline.ConfigHash, (baseline with { Calendar = new Calendar(360, 12) }).ConfigHash);
    }
    ...
    /// <summary>
    /// A world that keeps the standard year hashes exactly as it did before there was a calendar.
    /// </summary>
    /// <remarks>
    /// The same guarantee <c>RasterTerrainTests</c> pins for an empty <c>TerrainSource</c>, and it
    /// matters more here: every world ever generated by this engine kept a 360-day year implicitly,
    /// so a calendar hashed unconditionally would have restamped the provenance of every file on
    /// disk without a single history having changed.
    /// </remarks>
    [Fact]
    public void TheStandardCalendarDoesNotEnterTheHash()
    {
        var baseline = new WorldConfig();

        Assert.Equal(
            baseline.ConfigHash, (baseline with { Calendar = new Calendar() }).ConfigHash);
        Assert.Equal(
            baseline.ConfigHash, (baseline with { Calendar = new Calendar(360, 4) }).ConfigHash);
    }
